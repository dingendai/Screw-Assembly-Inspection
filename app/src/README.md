...
- 操作員登入：可先調整 GUI 顯示模式，輸入姓名並使用相機 5 拍攝操作員照片。
- 操作員登出：記錄登入與登出時間。
- 檢測攝影機設定：支援相機 1-4，自訂設備索引與啟用數量。
- 攝影機搜尋：可重新掃描新接上的攝影機。
- 四面影像檢測：相機 1-4 可對應 `front / back / left / right` 四個檢測視角。
- 多模型設定：可依每台檢測攝影機指定不同 YOLO 模型。
- 模型自動搜尋：目前程式會搜尋 `modles/` 下的 YOLO 權重檔。
- 即時預覽：登入頁與設定頁可預覽攝影機畫面。
- 影像方向設定：支援水平鏡像、垂直翻轉，以及 0/90/180/270 度旋轉。
- GUI 顯示設定：支援全螢幕、自動適應目前螢幕，以及指定視窗寬高。
- 檢測模式：監控頁支援單次檢測與連續檢測。
...
1. 啟動程式：相機 5 操作員預覽與相機 1-4 檢測預覽啟動。
2. 操作員登入：輸入操作員姓名並拍攝操作員照片。
3. 攝影機設定：選擇設備索引、啟用數量、影像方向，以及各攝影機使用的模型。
4. GUI 顯示設定：登入頁或相機設定頁都可選擇全螢幕、自動適應目前螢幕，或指定 GUI 視窗寬高。
5. 套用設定：設定會寫入 `inspection_data/app_config.json`。
6. 進入監控：可執行單次或連續檢測。
7. 查看歷史：檢視檢測紀錄與操作員登入紀錄。
8. 操作員登出：登出紀錄會寫入 `inspection_data/operator_sessions.csv`。
...
└─ valve_gui/
   ...
   └─ pages/
      ├─ login.py                  # 操作員登入與相機 5 拍照
      ├─ settings.py               # 攝影機啟用、順序、預覽、模型指定
      ├─ monitor.py                # 相機 1-4 監控與檢測
...
程式會依 `AppState.inspection_cameras` 中的攝影機設定，把不同相機 slot 的 frame 分派到對應模型。
